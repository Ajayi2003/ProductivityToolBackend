...
export const readTask = async(req:Request, res:Response):Promise<Response>=>{
    try {
        const task = await ProgressModel.find()
...
export const readOneTask = async(req:Request, res:Response):Promise<Response>=>{
    try {
        const {id} = req.params
        const task = await ProgressModel.findById(id)
        return res.status(200).json({
            message: "Task gotten",
            data: task
        })
...
export const CreateTask = async(req:Request<{},{}, iTask>, res:Response):Promise<Response>=>{
    try {
        ...
        const create = await ProgressModel.create({
            task,
            priority,
            isCompleted: false,
            image
...
export const DeleteTask = async(req:Request, res:Response):Promise<Response>=>{
    try {
        const {id} = req.params
        const remove = await ProgressModel.findByIdAndDelete(id)
        return res.status(200).json({
...
export const UpdateTask = async(req:Request, res:Response):Promise<Response>=>{
    try {
        ...
        const update = await ProgressModel.findByIdAndUpdate(req.params.id, {isCompleted: true}, {new:true})
        return res.status(201).json({
